...
class PrettyDioLogger extends Interceptor {
  ...
  void _printRequestHeader(RequestOptions options) {
    ...
    _printInfo('');
    _printInfo('HTTP REQUEST STARTED');
    _printInfo('');
    _printBoxed(header: 'Request  $method ', text: uri.toString());
  }

  void _printLine() => logPrint('═' * maxWidth);

  void _printInfo(String text) {
    final lenght = maxWidth - text.length;
    final charLneght = (lenght / 2).round();
    final logChars = '=' * charLneght;
    logPrint(logChars + text + logChars);
  }
